...
1. Create your own repository:
    ...
    - In local:
        ```bash
        user@host$ mkdir <bitbucket_reponame>
        user@host$ cd <bitbucket_reponame>
        user@host$ git init
        ```
    ...
    - In Bitbucket:
        ...
        2. Clone repository:
            ```bash
            user@host$ git clone ssh://git@globaldevtools.bbva.com:7999/~user/<bitbucket_reponame>.git
            ```
        3. Enter directory:
            ```bash
...
2. Enter your repository and run the following command:
    ```bash
    git archive --format=tar --remote=ssh://git@globaldevtools.bbva.com:7999/~alejandro.aguayo.ortiz/python-project-template.git HEAD ":(exclude).misc/" | tar xf -
    ```

    If you are getting the following error: `fatal: operation not supported by protocol` it means that your Bitbucket server not longer accepts SSH protocol, and git archive does not work with HTTP. In that case, these are the alternative steps. 

    1. Exit the directory of your repository and clone this one:
        ```bash
        user@host$ git clone https://bitbucket.globaldevtools.bbva.com/bitbucket/scm/~alejandro.aguayo.ortiz/python-project-template.git
        ```
...
4. Follow the Cookiecutter steps by defining:
    - Bitbucket username [`bitbucket_username`]: The username associated with your
    Bitbucket account (e.g., *alejandro.aguayo.ortiz*).
    - Bitbucket repository name [`bitbucket_reponame`]: The name given to the project
    repository while creating it (e.g., *awesome-project*).
    - Author [`author`]: Author's full name. Default value obtained from
    `bitbucket_username` (e.g., *Alejandro Aguayo Ortiz*).
    - Email [`email`]: Email associated with Bitbucket account. Default value obtained from
    `bitbucket_username` (e.g., *alejandro.aguayo.ortiz@bbva.com*).
    - Project Name [`project_name`]: The full name given to the project. Default value
    obtained from `bitbucket_reponame` (e.g., *Awesome Project*).
...
[[**alejandro.aguayo.ortiz@bbva.com**](mailto:alejandro.aguayo.ortiz@bbva.com)].

## Demo video

Click in this image to see a demo of the usage of this template and its features:

[![Watch demo video](.misc/demo.png)](https://bitbucket.globaldevtools.bbva.com/bitbucket/users/alejandro.aguayo.ortiz/repos/python-project-template/browse/.misc/demo.mp4?raw=)
